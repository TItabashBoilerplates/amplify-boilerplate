...
理由: `devenv:enterShell` hook を経由しないと `setup:install-frontend` / `setup:install-backend` 等（`before = [ "devenv:enterShell" ]` で登録された task）が発火せず、`bun install --frozen-lockfile` / `uv sync --frozen` が走らないため、`turbo` / `eslint` / `tsc` 等の node_modules / .venv 依存ツールが PATH 上に出現しない状態で verify task が実行されて落ちる。
...
- name: Run verify tasks
  run: |
    devenv tasks run \
      lint-ci:frontend lint-ci:backend-py lint-ci:fsd \
      format-check:frontend format-check:backend-py \
      type-check:frontend type-check:mobile type-check:backend-py
```
...
- `backend` / `storybook` プロセスを立ち上げようとする

CI で lint / format / type-check しか走らせたくないのに毎回 backend / Storybook を起動するのはコスト・時間の無駄。だから `devenv test` の代わりに、aggregator 配下の verify task を `devenv tasks run` で**直接列挙**する。

> Amplify backend（AppSync/DynamoDB/Cognito/S3/Lambda）はクラウド sandbox（`ampx sandbox`）であり、ローカル Docker を立てる process は存在しない。CI の検証フェーズでバックエンドを起動する必要はない（デプロイは Amplify Hosting が `amplify.yml` に従って `ampx pipeline-deploy` で実行する）。
...
| `actions/cache@v4` の `path: frontend/**/node_modules` | **Bun workspace の install 結果**（`/nix/store` 外。`@workspace/backend` の Amplify 定義も `frontend/packages/backend/node_modules` に入る） | `setup:install-frontend` task が `execIfModified` で skip された際に node_modules が空になるのを防ぐ |
...
- name: Cache node_modules
  ...
  with:
    ...
    key: ${{ runner.os }}-node-modules-${{ hashFiles('frontend/bun.lock') }}
    restore-keys:
...
```yaml
# ❌ NG: process phase で backend / Storybook が毎回起動する
- run: devenv test
```
...
- [ ] `actions/cache` の `path:` に **`frontend/**/node_modules` 等の install 結果が含まれているか**
...
**修正（暫定）**: cache `path:` に `frontend/node_modules` 系（`@workspace/backend` 等の workspace パッケージを含む）を追加。lockfile が変われば cache key も変わるので整合性も取れる。
