...
| Amplify × 広域 AWS（SQS / Bedrock / EventBridge / Step Functions / 追加 DynamoDB・S3 / カスタム CDK / IAM 付与） | `amplify-gen2`（最優先・`references/aws-services.md`）, `langchain`（Bedrock+LangChain時） |
| 生成AI / LLM / チャット / ストリーミング(SSE) / AIエージェント / バックグラウンド処理 + ステータス監視 / **AgentCore（超長時間・サンドボックス）** | `amplify-gen2`（最優先・`references/generative-ai.md`）, `langchain`, `data-fetching` |
| Next.js / App Router / Server Components / Cache | `nextjs`, `next-cache-components`, `next-upgrade` |
...
| デスクトップアプリ / Tauri / IPC / capabilities / CSP / WebView / `apps/desktop` | **`tauri`**（自作）+ `monorepo` |
