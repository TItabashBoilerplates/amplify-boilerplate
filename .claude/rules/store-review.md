...
| 検査 | 落ちたときに防いでいる事故 |
|---|---|
| ログイン画面がパスワード認証（`signIn({ username, password })`）を使っている | OTP のみ → **2.1(a) でリジェクト** |
...
- `docs/store/submission-checklist.md` / `docs/store/release-runbook.md`
- `.claude/skills/store-screenshots/` / `.claude/skills/mobile-release/`
