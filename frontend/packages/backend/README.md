...
packages/backend/
├── amplify/
│   ├── backend.ts            # defineBackend({ auth, data, storage, api }) + FastAPI Lambda / SNS 配線
...
| **api (FastAPI Lambda)** | `functions/api/resource.ts` | CDK `Function`(PYTHON_3_13)。リポジトリ root の `backend-py`（uv workspace）をバンドルし、ハンドラ `api.lambda_handler.handler`（Mangum）で FastAPI を Lambda に適合。認可は Cognito JWT 検証。 |
