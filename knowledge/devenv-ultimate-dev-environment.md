...
```nix
"lint-ci:frontend" = {
  exec = ''cd frontend && pnpm run lint:ci'';
...
"setup:install-frontend" = {
  exec = ''cd frontend && pnpm install --frozen-lockfile'';
  execIfModified = [
...
`execIfModified` のおかげで「lockfile に変更が無いときはスキップ」されるので、毎回ターミナルを開くたびに `pnpm install` が走ることはありません。`--frozen-lockfile` で lockfile の意図しない書き換えも防止しています。
...
AI が `package.json` に新しい依存を追加しても、次に `cd` した瞬間に `setup:install-frontend` task が走って `pnpm install --frozen-lockfile` が実行されます。**AI が `pnpm install` を叩き忘れるという事故が構造的に発生しません**。
...
`commands.md` には「品質チェックは必ず devenv の scripts を使うこと（直接 `pnpm run biome check` するのは禁止）」と書かれています。AI はこのルールを読み込むので、**「環境構築のお作法」が会話のたびに崩れる事態を防げる**。
...
├── rules/
│   ├── commands.md                    # ← 「devenv コマンド必須、直接 pnpm run biome 禁止」
│   ├── skills-first.md
│   ├── tdd.md
│   └── mcp-supabase.md
└── memory/
...
>
> **Why:** 元々 Makefile を使っていたが、2026-04 に devenv tasks/scripts への一本化を実施。直接 `pnpm run biome`, `uv run ruff`, `npx tsc` 等を叩くと環境差異・CI 不整合・profile (env) 未読み込みのリスクがある。
>
> **How to apply:** `pnpm run biome`, `uv run ruff`, `npx tsc` 等を直接実行しない。代わりに `lint`, `format`, `type-check`, `ci-check` などの devenv scripts を使う。
...
- ✅ **自己修復性**：`setup:*` task が lockfile 変更を自動検知して `pnpm install` / `uv sync`
